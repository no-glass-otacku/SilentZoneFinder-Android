...
class LoginActivity : AppCompatActivity() {
    ...
    private suspend fun testDatabaseConnection() {
        ...
        try {
            ...
            if (session != null) {
                val userId = session.user?.id ?: "알 수 없음"
                Toast.makeText(this, "DB 연결 성공! 사용자 ID: $userId", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(this, "세션을 가져올 수 없습니다.", Toast.LENGTH_SHORT).show()
            }
        } catch (e: Exception) {
            Toast.makeText(this, "DB 연결 테스트 실패: 네트워크를 확인해주세요.", Toast.LENGTH_SHORT).show()
        }
    }
}
